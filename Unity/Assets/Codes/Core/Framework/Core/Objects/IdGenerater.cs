...
namespace Framework
{
    ...
    public class IdGenerater: IDisposable
    {
        ...
        public IdGenerater()
        {
            ...
            {
                CustomLogger.Log(LoggerLevel.Warning,$"lastInstanceIdTime less than 0: {this.lastInstanceIdTime}");
                this.lastInstanceIdTime = 1;
            }
            this.lastIdTime = TimeSince2020();
            if (this.lastIdTime <= 0)
            {
                CustomLogger.Log(LoggerLevel.Warning,$"lastIdTime less than 0: {this.lastIdTime}");
                this.lastIdTime = 1;
            }
            this.lastUnitIdTime = TimeSince2020();
            if (this.lastUnitIdTime <= 0)
            {
                CustomLogger.Log(LoggerLevel.Warning,$"lastUnitIdTime less than 0: {this.lastUnitIdTime}");
        ...
        {
            uint a = (uint)((GameWorld.TimeInfo.FrameTime - this.epoch2020) / 1000);
            return a;
        }
        
        private uint TimeSinceThisYear()
        {
            uint a = (uint)((GameWorld.TimeInfo.FrameTime - this.epochThisYear) / 1000);
            return a;
        }
        ...
        public long GenerateInstanceId()
        {
            ...
            else
            {
                ...
                if (this.instanceIdValue > IdGenerater.Mask18bit - 1) // 18bit
                {
                    ...
                    CustomLogger.Log(LoggerLevel.Error,$"instanceid count per sec overflow: {time} {this.lastInstanceIdTime}");
                }
            }
        ...
        public long GenerateId()
        {
            ...
            else
            {
                ...
                if (value > ushort.MaxValue - 1)
                {
                    ...
                    CustomLogger.Log(LoggerLevel.Error,$"id count per sec overflow: {time} {this.lastIdTime}");
                }
            }
        ...
        public long GenerateUnitId(int zone)
        {
            ...
            else
            {
                ...
                if (this.unitIdValue > ushort.MaxValue - 1)
                {
                    ...
                    CustomLogger.Log(LoggerLevel.Error,$"unitid count per sec overflow: {time} {this.lastUnitIdTime}");
                }
            }
